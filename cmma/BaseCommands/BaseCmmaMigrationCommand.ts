...
import { CmmaMigratorContract } from '../TypeChecking/Migration/CmmaMigratorContract'
...
export abstract class BaseCmmaMigrationCommand extends BaseCmmaCommand {
   ...
   */
  private logVerboseFinalStatus(migrator: CmmaMigratorContract, duration?: [number, number]) {
    ...
    switch (migrator.status) {
      ...
      case 'error':
        const skippedMigrations = Object.values(migrator.migratedFiles).filter(
          (file: MigratedFileNode) => file.status === 'pending'
        ).length
